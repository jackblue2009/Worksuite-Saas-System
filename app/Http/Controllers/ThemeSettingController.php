...
class ThemeSettingController extends AccountBaseController
{
    ...
    public function store(UpdateThemeSetting $request)
    {
        ...
        if (!$superAdminThemeSetting->restrict_admin_theme_change){
            ...
            $this->themeUpdate($adminTheme, $request->theme_settings[1], $request->text_color[0], $request->primary_color[0]);

            $employeeTheme = ThemeSetting::where('panel', 'employee')->first();
            $this->themeUpdate($employeeTheme, $request->theme_settings[3], $request->text_color[1], $request->primary_color[1]);

            $clientTheme = ThemeSetting::where('panel', 'client')->first();
            $this->themeUpdate($clientTheme, $request->theme_settings[4], $request->text_color[2], $request->primary_color[2]);
        }
        ...
        $setting->text_color = $request->global_text_color;
    ...
    private function themeUpdate($updateObject, $themeSetting, $textColor, $primaryColor)
    {
        $updateObject->header_color = $primaryColor;
        $updateObject->text_color = $textColor;
